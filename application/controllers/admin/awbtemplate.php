...
class Awbtemplate extends Application
{
    ...
    public function listing()
    {
        ...
        $this->table->set_heading(
            ...
            'Logistic AWB'); // Setting headings for the table
        ...
        $this->table->set_footing(
            ...
            '<input type="text" name="search_delivery_id" id="search_delivery_id" value="Search delivery ID" class="search_init" />',
            '<input type="text" name="search_merchant_trans_id" value="Search merchant_trans_id" class="search_init" />',
            '<input type="text" name="search_fullfilment" value="Search fullfilment" class="search_init" />'
            );

        $page['sortdisable'] = '';
        $page['ajaxurl'] = 'admin/awbtemplate/ajaxmanage';
        $page['add_button'] = array('link'=>'admin/members/add','label'=>'Add New Member');
        $page['page_title'] = 'AWB Template';
        $this->ag_auth->view('awbtemplate',$page); // Load the view
    }
    ...
    public function ajaxmanage(){
        ...
        $date_from = $in['fromDate'];
        $date_to = $in['toDate'];

        $merchant_id = $in['logisticId'];
        ...
        $this->db->where($inc.'.created >=', $date_from.' 00:00:00');
        $this->db->where($inc.'.created <=', $date_to.' 23:59:59');
        ...
        if($this->input->post('sSearch_0') != ''){
            $this->db->like('members.merchant_name',$this->input->post('sSearch_0'));
            $search = true;
        }
        ...
        if($this->input->post('sSearch_3') != ''){
            $this->db->like('fulfillment_code',$this->input->post('sSearch_3'));
            $search = true;
        }
